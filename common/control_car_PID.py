...
# TODO: Learn how to tune PID parameters and do it
...
def move_forward(forward_speed):
    ...
    pid_controller = PID_controller(
        p=6.1, i=0, d=2.7, sp_p=1.30, sp_d=0.02, sp_i=0.40, desired_speed=forward_speed
    )  # params for curve
    rospy.Subscriber(
        "/pacmod/as_tx/vehicle_speed", Float64, pid_controller.speed_control
    )
    enable_pub.publish(Bool(enabled))
...
if __name__ == "__main__":
    rospy.init_node("control_car_PID")

    const_speed = 0.45
    move_forward(const_speed)
